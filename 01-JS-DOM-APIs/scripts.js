...
function getRepositories(){
	...
	let promise = httpReq2(searchText);

	promise.then(function(content){

		let list = "<ul>";

		for(let i in JSON.parse(content).items){  /* https://developer.mozilla.org/es/docs/Web/JavaScript/Referencia/Sentencias/for...in*/
...
function createTable(){
    ...
    for(let i = 0; i < 6; i++)
    {
      ...
      for(let j = 0; j < 6; j++){
        ...
        let cellContent = document.createTextNode(matrix[i][j]);
